...
import { Box as ChakraBox } from "@chakra-ui/react"; // Import Box from Chakra UI
...
function SideDrawer() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`/api/user?search=${search}&filter=${selectedFilter}`, config);
    ...
    <>
      ...
      <Drawer placement="left" onClose={onClose} isOpen={isOpen}>
        ...
        <DrawerContent>
          ...
          <DrawerBody>
            <Box d="flex" pb={2}>
              <ChakraBox position="relative" mr={2}>
                <select
